...
class PGVectorProvider(VectorDBInterface):
    ...
    async def search_by_vector(
        ...
        if file_chapter_filters:
            ...
            for item in file_chapter_filters:
                ...
                item_conditions = []
                if fid is not None:
                    fid_clean = str(fid).replace("'", "''")
                    item_conditions.append(f"{PgVectorTableSchemeEnums.METADATA.value}->>'file_id' = '{fid_clean}'")
                if ch is not None:
                    ch_clean = str(ch).replace("'", "''")
                    item_conditions.append(f"{PgVectorTableSchemeEnums.METADATA.value}->>'chapter_title' = '{ch_clean}'")
                    
                if item_conditions:
                    conditions.append(f"({' AND '.join(item_conditions)})")
                    
            if conditions:
                filter_clause = f" WHERE ({' OR '.join(conditions)})"
        elif chapters:
            escaped_chapters = [c.replace("'", "''") for c in chapters]
            chapters_in = ", ".join(f"'{c}'" for c in escaped_chapters)
            filter_clause = f" WHERE {PgVectorTableSchemeEnums.METADATA.value}->>'chapter_title' IN ({chapters_in})"
    ...
    async def search_by_keyword(
        ...
        """
        Performs pure keyword/full-text search using PostgreSQL ts_rank scoring.
        Results represent BM25-style text relevance for RRF fusion.

        Args:
            collection_name: Target PGVector table name.
            keyword_query: The raw keyword query string.
            limit: Maximum results to return.
            chapters: Optional chapter title filter.
            file_chapter_filters: Optional list of file/chapter filter dicts.

        Returns:
            List of RetrievedDocument with ts_rank scores.
        """
        is_collection_existed = await self.is_collection_existed(
            collection_name=collection_name
        )
        if not is_collection_existed:
            self.logger.error(
                f"Cannot search keyword in non-existent collection: {collection_name}"
            )
            return []
        ...
        if file_chapter_filters:
            ...
            chapters_in = ", ".join(f"'{c}'" for c in escaped_chapters)
            filter_clause = (
                f" AND {PgVectorTableSchemeEnums.METADATA.value}->>'chapter_title' IN ({chapters_in})"
            )

        async with self.db_client() as session:
            async with session.begin():
                keyword_sql = sql_text(
                    f"SELECT {PgVectorTableSchemeEnums.TEXT.value} as text, "
                    f"ts_rank(to_tsvector('english', {PgVectorTableSchemeEnums.TEXT.value}), "
                    f"plainto_tsquery('english', :query)) as score "
                    f"FROM {collection_name} "
                    f"WHERE to_tsvector('english', {PgVectorTableSchemeEnums.TEXT.value}) "
    ...
    async def get_random_documents(
        ...
        """Fetches random documents matching the given filters, ignoring semantic/keyword searches."""
        is_collection_existed = await self.is_collection_existed(
            collection_name=collection_name
        )
        if not is_collection_existed:
            self.logger.error(f"Cannot fetch from non-existent collection: {collection_name}")
            return []
        ...
        async with self.db_client() as session:
            async with session.begin():
                ...
                result = await session.execute(random_sql)
                records = result.fetchall()

                return [
                    RetrievedDocument(text=record.text, score=1.0)
                    for record in records
                ]
